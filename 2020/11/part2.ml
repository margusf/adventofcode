...
let solve input_lst =
    ...
    let count_full arr row col =
        ...
        let rec count_visible row col ret (drow, dcol) =
            let new_row = row + drow in
            let new_col = col + dcol in
            if out_of_bounds new_row new_col then
                ret
            else
                match arr.(new_row).(new_col) with
                | Floor -> count_visible new_row new_col ret (drow, dcol)
                | Empty -> ret
                | Full -> ret + 1 in

        List.fold_left (count_visible row col) 0 neighbours in
        ...
        let transform row col =
            ...
            | Empty ->
                if (count_full input row col) = 0 then
                    ...
                    Empty
            | Full ->
                if (count_full input row col) >= 5 then
                    Empty
                else
                    Full
            | Floor -> Floor in
