...
let input_test = (
    [
        (0, [[Rule(4); Rule(1); Rule(5); ]]);
        (1, [[Rule(2); Rule(3);  ]; [Rule(3); Rule(2); ]]);
        (2, [[Rule(4); Rule(4);  ]; [Rule(5); Rule(5); ]]);
        (3, [[Rule(4); Rule(5);  ]; [Rule(5); Rule(4); ]]);
        (4, [[ Lit('a')]]);
        (5, [[ Lit('b')]]);
    ],
    [
...
let solve (rule_list, string_list) =
    ...
    let rec do_match rule_idx str idx =
        let rec try_opt rule_list idx =
            let do_rule rule idx =
                match rule with
                | Rule(rule_idx) -> do_match rule_idx str idx
                | Lit(c) ->
                    if c = String.get str idx then
                        Some(idx + 1)
                    else
                        None in
        ...
        let opt_list = IntMap.find rule_idx rules in
        match_optlist opt_list idx in

    let string_matches str =
        let ret = do_match 0 str 0 in
        match ret with
        | None -> false
        | Some(idx) -> idx = String.length str in

    List.length (List.filter string_matches string_list)
;;
