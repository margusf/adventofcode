...
let solve input =
    let rec read pos =
        let next_pos = {pos with pos = pos.pos + 1} in
        if pos.pos >= String.length pos.str then
            (next_pos, '#')
        else if String.get pos.str pos.pos = ' ' then
            read next_pos  (* Skip whitespace *)
        else begin
            printf "%c\n" (String.get pos.str pos.pos);
            (next_pos, String.get pos.str pos.pos)
        end in

    let push_back pos =
        {pos with pos = pos.pos - 1} in

    let eval str =
        let rec eval_arg indent pos =
            printf "%sarg\n" indent;
            let (pos1, c) = read pos in
            match c with
            | '0'..'9' -> (pos1, (int_of_char c) - (int_of_char '0'))
            | '(' -> eval_expr (indent ^ "  ") pos1
            | c -> raise (Invalid_argument ("expected arg: " ^ (Char.escaped c)))

        and eval_addop indent pos prev =
            let (pos1, op) = read pos in
            printf "%saddop %c\n" indent op;
            if op = '#' || op = ')' then
                (push_back pos1, prev) (* Have to fall out of mulop as well. *)
            else if op = '*' || op = '/' then
                (push_back pos1, prev)
            else
                let (pos2, arg) = eval_arg (indent ^ "  ") pos1 in
                let op_result =
                    match op with
                    | '+' -> prev + arg
                    | '-' -> prev - arg
                    | c -> raise (Invalid_argument ("expected addop: " ^ (Char.escaped c))) in
                eval_addop (indent ^ "  ") pos2 op_result

        and eval_addexpr indent pos =
            printf "%saddexpr\n" indent;
            let (pos1, arg) = eval_arg (indent ^ "  ") pos in
            eval_addop (indent ^ "  ") pos1 arg

        and eval_mulop indent pos prev =
            let (pos1, op) = read pos in
            printf "%smulop %c\n" indent op;
            if op = '#' || op = ')' then
                (pos1, prev)
            else
                let (pos2, arg) = eval_addexpr (indent ^ "  ") pos1 in
                let op_result =
                    match op with
                    | '*' -> prev * arg
                    | '/' -> prev / arg
                    | ')' -> prev
                    | c -> raise (Invalid_argument ("expected mulop: " ^ (Char.escaped c))) in
                eval_mulop (indent ^ "  ") pos2 op_result

        and eval_expr indent pos =
            printf "%seval_expr\n" indent;
            let (pos1, arg) = eval_addexpr (indent ^ "  ") pos in
            eval_mulop (indent ^ "  ") pos1 arg in
