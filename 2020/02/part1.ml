...
let valid_passwords lst =
    let count_lst f lst =
        length (filter f lst) in
    let count_str ch str =
        let count = ref 0 in
        let countif c =
            if c = ch then
                count := !count + 1
                ...
                () in
        String.iter countif str;
        !count in
    let pw_matches (min, max, c, pw) =
        let char_count = count_str c pw in
        (char_count >= min) && (char_count <= max)
    in
    count_lst pw_matches lst
;;
